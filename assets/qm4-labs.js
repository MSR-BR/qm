...
\\qquad -|V_0|<E_n<0
\\]\\[
\\nu=\\left\\lceil\\frac{k_0a}{\\pi}\\right\\rceil,\\qquad
\\frac{E_n}{|V_0|}=\\left(\\frac{\\tilde k_na}{k_0a}\\right)^2-1,\\qquad
\\tilde\\rho_n(x)=\\frac{\\rho_n(x)}{20|C|^2}
\\]` },
    scatter: { slug:"attractive-well-scattering", title:"Attractive finite well: transmission resonances", subtitle:"The incident energy is fixed at \\(E/|V_0|=0.10\\). Vary the book parameter \\(k_0a/\\pi\\) to see how the transmission coefficient changes.", sections:"4.9–4.10", show:["k0a"], main:"Transmission as the well parameter changes", side:"", equation:"\\[k^2=\\frac{2mE}{\\hbar^2},\\qquad \\bar k^2=k_0^2+k^2\\]\\[T_{k_0a}(ka)=\\left[1+\\frac{(k_0a)^4}{4(ka)^2\\bigl[(k_0a)^2+(ka)^2\\bigr]}\\sin^2\\!\\left(\\sqrt{(k_0a)^2+(ka)^2}\\right)\\right]^{-1},\\qquad R=1-T\\]" },
  ...
  const scatterEnergyRatio = .10;
  function scatterTransmission(k0a) {
    const ka2=scatterEnergyRatio*k0a*k0a;
    return 1/(1+(k0a**4/(4*ka2*(k0a*k0a+ka2)))*Math.sin(Math.sqrt(k0a*k0a+ka2))**2);
  }
  function drawMain(s, type) {
    const out=fit($("mainCanvas")),ctx=out.ctx,w=out.width,h=out.height,p={l:52,r:22,t:24,b:34},mid=h*.48;
    if(type!=="qho"&&type!=="finite"&&type!=="scatter")axes(ctx,w,h,p,mid);
    if(type==="qho") {
    ...
    if(type==="scatter") {
      const base=h-p.b,top=p.t,xm=r=>p.l+(r/6.5)*(w-p.l-p.r),ym=t=>base-t*(base-top);
      axes(ctx,w,h,p,base);
      ctx.strokeStyle=C.grid;ctx.lineWidth=1;
      [.25,.5,.75,1].forEach(t=>{ctx.beginPath();ctx.moveTo(p.l,ym(t));ctx.lineTo(w-p.r,ym(t));ctx.stroke();});
      for(let n=1;n<=6;n++) {
        const resonance=n/Math.sqrt(1+scatterEnergyRatio);
        if(resonance>6.5) continue;
        ctx.strokeStyle="#b9d8c6";ctx.setLineDash([4,5]);ctx.beginPath();ctx.moveTo(xm(resonance),top);ctx.lineTo(xm(resonance),base);ctx.stroke();ctx.setLineDash([]);
      }
      drawCurve(ctx,r=>scatterTransmission(r*Math.PI),800,xm,ym,C.green,.02,6.5);
      const selected=s.k0a/Math.PI,T=scatterTransmission(s.k0a);
      ctx.strokeStyle=C.rust;ctx.lineWidth=1.5;ctx.setLineDash([5,4]);ctx.beginPath();ctx.moveTo(xm(selected),top);ctx.lineTo(xm(selected),base);ctx.stroke();ctx.setLineDash([]);
      ctx.fillStyle=C.rust;ctx.beginPath();ctx.arc(xm(selected),ym(T),5,0,2*Math.PI);ctx.fill();
      label(ctx,"k₀a / π",w*.49,h-8);label(ctx,"T",p.l+8,17,12,C.ink);
      label(ctx,"perfect-transmission values",Math.min(w-170,xm(4.25)),top+15,10,C.muted);
      return;
    }
  ...
  function render() {
    ...
    controls.a.max="4";
    controls.k0a.min=key==="scatter"?"0.1":"1.1";
    ...
    sidePanel.style.display=(key==="qho"||key==="finite"||key==="scatter")?"none":"";
    metricsPanel.style.display=(key==="qho"||key==="finite"||key==="scatter")?"none":"";
    document.querySelector(".sim-footer").textContent=key==="finite"?"Finite-well graph in the book variables x/a and Eₙ/|V₀|. The displayed density follows ρ̃ₙ(x)=ρₙ(x)/(20|C|²).":key==="scatter"?"Transmission coefficient in the dimensionless variables used in the book. The energy ratio E/|V₀| is held fixed while k₀a/π is varied.":"SI constants are used internally. Lengths are entered in nm, energies displayed in eV, and angular frequencies in 10¹⁵ s⁻¹.";
    ...
    if(key==="qho") {
      ...
      const T=scatterTransmission(s.k0a);
      $("insight").textContent="At k₀a/π = "+decimal(s.k0a/Math.PI,2)+", T = "+decimal(T,3)+" and R = "+decimal(1-T,3)+". Perfect transmission occurs when sin²(k̄a) vanishes; the dashed lines mark those values for the fixed ratio E/|V₀| = 0.10.";
      legend([[C.green,"transmission coefficient T"],[C.rust,"selected k₀a/π"],["#b9d8c6","sin²(k̄a) = 0" ]]);
    } else if(key==="delta") {
    ...
    drawMain(s,key);if(key!=="qho"&&key!=="finite"&&key!=="scatter")drawSide(s,key);
    typesetLabMath();
  }
  render();
})();
