  ...
  function rewriteChapterFive() {
    ...
    const pages = {
      "central-potentials-chapter-roadmap": {
        guide: "This chapter treats central potentials, for which \(V(\mathbf r)=V(r)\). The common angular problem is separated first; the radial equation is then solved separately for hydrogen and for the three-dimensional harmonic oscillator.",
        content: `<div class="col"><div class="card"><div class="ch"><i class="fa-solid fa-circle-dot"></i> Central-potential Hamiltonian</div><div class="eq key-eq">\\[\\hat H=\\frac{\\hat p^2}{2m}+V(\\hat r),\\qquad \\hat H\\Psi(\\mathbf r)=E\\Psi(\\mathbf r)\\]</div><p>The potential depends only on the distance from the origin. Spherical coordinates therefore make the geometry of the problem explicit.</p></div><div class="card green"><div class="ch green"><i class="fa-solid fa-code-branch"></i> The common separation</div><div class="eq">\\[\\Psi(r,\\theta,\\phi)=R(r)\\Theta(\\theta)\\Phi(\\phi)\\]</div><p>The angular factors are determined once through the spherical harmonics. The radial factor changes when the potential changes.</p></div></div><div class="col"><div class="card orange"><div class="ch orange"><i class="fa-solid fa-list-ol"></i> Reading sequence</div><ol class="numbered"><li>Write the time-independent Schrödinger equation in spherical coordinates.</li><li>Separate its radial and angular parts.</li><li>Obtain the allowed angular functions from periodicity and regularity.</li><li>For each potential, solve the corresponding radial equation and impose normalizability.</li></ol></div><div class="card purple"><div class="ch purple"><i class="fa-solid fa-atom"></i> Two radial problems</div><p>For hydrogen the Coulomb attraction gives the Bohr scale and the spectrum \(E_n=-E_0/n^2\). For the isotropic oscillator, the quadratic potential gives equally spaced energies. The book develops both cases after the shared angular analysis.</p></div></div>`,
        keys: ["\\Psi(r,\\theta,\\phi)=R(r)\\Theta(\\theta)\\Phi(\\phi)", "\\hat H=\\hat p^2/(2m)+V(\\hat r)"]
      },
      "separation-of-variables-in-spherical-coordinates": {
        guide: "Start from the time-independent Schrödinger equation for \(V(r)\). In spherical coordinates, substituting a product \(R(r)\Theta(\theta)\Phi(\phi)\) separates a radial expression from an angular expression.",
        content: `<div class="card"><div class="ch"><i class="fa-solid fa-square-root-variable"></i> Time-independent Schrödinger equation</div><div class="eq key-eq">\\[\\nabla^2\\Psi+\\frac{2m}{\\hbar^2}[E-V(r)]\\Psi=0\\]</div><p>The spherical Laplacian has one radial term and two angular terms:</p><div class="eq">\\[\\nabla^2=\\frac1{r^2}\\frac{\\partial}{\\partial r}\\left(r^2\\frac{\\partial}{\\partial r}\\right)+\\frac1{r^2\\sin\\theta}\\frac{\\partial}{\\partial\\theta}\\left(\\sin\\theta\\frac{\\partial}{\\partial\\theta}\\right)+\\frac1{r^2\\sin^2\\theta}\\frac{\\partial^2}{\\partial\\phi^2}.\\]</div></div><div class="col"><div class="card green"><div class="ch green"><i class="fa-solid fa-arrow-right-long"></i> Radial equation</div><div class="eq">\\[\\frac1{r^2}\\frac{d}{dr}\\left(r^2\\frac{dR}{dr}\\right)+\\left[\\frac{2m}{\\hbar^2}(E-V)-\\frac{\\lambda}{r^2}\\right]R=0.\\]</div></div><div class="card purple"><div class="ch purple"><i class="fa-solid fa-compass"></i> Angular equation</div><div class="eq">\\[\\frac1{\\Theta\\sin\\theta}\\frac{d}{d\\theta}\\left(\\sin\\theta\\frac{d\\Theta}{d\\theta}\\right)+\\frac1{\\Phi\\sin^2\\theta}\\frac{d^2\\Phi}{d\\phi^2}+\\lambda=0.\\]</div><p>The separation constant \(\\lambda\) is fixed by the acceptable angular solutions in the next section.</p></div></div>`,
        keys: ["\\nabla^2\\Psi+2m[E-V(r)]\\Psi/\\hbar^2=0", "\\frac1{r^2}\\frac{d}{dr}(r^2R')+[2m(E-V)/\\hbar^2-\\lambda/r^2]R=0"]
      },
      "azimuthal-and-polar-equations": {
        guide: "The angular equation separates into an azimuthal equation for \(\Phi(\phi)\) and a polar equation for \(\Theta(\theta)\). The allowed constants are determined by the conditions that the wave function be single-valued and finite.",
        content: `<div class="col"><div class="card"><div class="ch"><i class="fa-solid fa-arrows-rotate"></i> Azimuthal contribution</div><div class="eq key-eq">\\[\\frac1{\\Phi}\\frac{d^2\\Phi}{d\\phi^2}=-m^2,\\qquad \\Phi_m(\\phi)=e^{im\\phi}.\\]</div><p>Since \(\\Phi(\\phi+2\\pi)=\\Phi(\\phi)\), the book obtains \(m=0,\\pm1,\\pm2,\\ldots\).</p></div><div class="card green"><div class="ch green"><i class="fa-solid fa-location-dot"></i> Polar contribution</div><p>With \(x=\\cos\\theta\) and \(\\Theta(\\theta)=P(x)\), the remaining equation becomes the associated Legendre equation:</p><div class="eq">\\[\\frac{d}{dx}\\left[(1-x^2)\\frac{dP}{dx}\\right]+\\left[\\lambda-\\frac{m^2}{1-x^2}\\right]P=0.\\]</div></div></div><div class="col"><div class="card orange"><div class="ch orange"><i class="fa-solid fa-check-double"></i> Regular solutions</div><p>The polynomial solutions require</p><div class="eq">\\[\\lambda=l(l+1),\\qquad l=0,1,2,\\ldots,\\qquad |m|\\leq l.\\]</div><p>The corresponding functions are the associated Legendre polynomials \(P_l^m(\\cos\\theta)\). The detailed construction is given in the book.</p></div><div class="card purple"><div class="ch purple"><i class="fa-solid fa-link"></i> Connection</div><p>The azimuthal phase and the polar polynomial are combined in the spherical harmonics on the following page.</p></div></div>`,
        keys: ["\\Phi_m=e^{im\\phi},\\;m=0,\\pm1,\\ldots", "\\lambda=l(l+1),\\;|m|\\le l"]
      },
      "spherical-harmonics": {
        guide: "The product of the polar and azimuthal solutions gives the spherical harmonics. They supply the angular contribution for every central-potential problem considered in this chapter.",
        content: `<div class="col"><div class="card"><div class="ch"><i class="fa-solid fa-globe"></i> Definition</div><div class="eq key-eq">\\[Y_l^m(\\theta,\\phi)=A_{lm}P_l^m(\\cos\\theta)e^{im\\phi},\\qquad A_{lm}=\\sqrt{\\frac{2l+1}{4\\pi}\\frac{(l-m)!}{(l+m)!}}.\\]</div><p>Here \(l=0,1,2,\\ldots\) and \(m=-l,\\ldots,l\). The normalization factor \(A_{lm}\) is chosen by the angular normalization condition.</p></div><figure class="book-figure"><img src="../../assets/chapter-05/figure-5-2-spherical-harmonics.png" alt="Spherical-harmonic probability patterns from the book" /><figcaption>Figure 5.2 from the book: probability patterns associated with low-order spherical harmonics. The labels \(s,p,d\) correspond to \(l=0,1,2\).</figcaption></figure></div><div class="col"><div class="card green"><div class="ch green"><i class="fa-solid fa-scale-balanced"></i> Orthogonality</div><div class="eq">\\[\\int_0^{2\\pi}\\!d\\phi\\int_0^\\pi\\!Y_l^{m*}(\\theta,\\phi)Y_{l'}^{m'}(\\theta,\\phi)\\sin\\theta\\,d\\theta=\\delta_{ll'}\\delta_{mm'}.\\]</div></div><div class="card orange"><div class="ch orange"><i class="fa-solid fa-puzzle-piece"></i> Role in the total state</div><div class="eq">\\[\\Psi_{nlm}(r,\\theta,\\phi)=R_{nl}(r)Y_l^m(\\theta,\\phi).\\]</div><p>The radial function is potential dependent; \(Y_l^m\) is the angular factor common to hydrogen and to the three-dimensional oscillator.</p></div></div>`,
        keys: ["Y_l^m=A_{lm}P_l^m(\\cos\\theta)e^{im\\phi}", "\\int Y_l^{m*}Y_{l'}^{m'}d\\Omega=\\delta_{ll'}\\delta_{mm'}"]
      },
