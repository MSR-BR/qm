...
export async function handleLegalPreferencesRequest({ method, headers = {}, body, env = process.env }) {
  if (!["GET", "PUT"].includes(method || "GET")) return jsonResponse(405, { error: "Use GET or PUT." });

  const { config, user } = await resolveUser(headers, env);
  if (!config) return jsonResponse(500, { error: "Preferences are not configured yet." });
  if (!user?.id) return jsonResponse(401, { error: "Sign in with Google to access your preferences." });

  if (method === "GET") {
    const result = await readRow(config, user.id);
    if (!result.ok) return jsonResponse(result.status || 500, { error: "Could not load your preferences." });
  ...
  if (!acceptDocuments && typeof emailUpdatesOptedIn !== "boolean") {
    return jsonResponse(400, { error: "Provide a valid change." });
  }
  ...
  if (!result.ok) return jsonResponse(result.status || 500, { error: "Could not save your preferences." });
