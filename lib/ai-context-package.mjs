...
export function buildAiExerciseContextPackage({ chapterId = "", itemId = "", pagePath = "", pageTitle = "", pageSubtitle = "", pageContent = "", difficulty = "medio" } = {}) {
  ...
  const context = {
    ...
    topicIndex: topicIndex ? { primaryTopic: topicIndex.primaryTopic, topicTags: topicIndex.topicTags || [], transversalTopics: topicIndex.transversalTopics, advancedSupportTopics: topicIndex.advancedSupportTopics, reviewStatus: topicIndex.reviewStatus || "" } : null,
...
export function buildAiExerciseContextPrompt(context = {}) {
  ...
  if (context.topicIndex) lines.push(`CURATED TOPIC POLICY — primary: ${context.topicIndex.primaryTopic}; reviewed map: ${context.topicIndex.reviewStatus || "curated"}; central links: ${context.topicIndex.transversalTopics.join(", ") || "none"}; advanced support: ${context.topicIndex.advancedSupportTopics.join(", ") || "none"}.`);
