...
import { buildAiExerciseContextPackage, buildAiExerciseContextPrompt } from "./ai-context-package.mjs";
import { summarizeMathContractResult, validateExerciseMathContract } from "./math-format-validator.mjs";
import { isChapterExerciseEligible } from "./qm-content-registry.mjs";
...
const EXERCISE_VALIDATION_ENABLED = true;

const plainMathPattern =
  /(?:\\begin\{(?:[pbBvV]?matrix|smallmatrix|array|cases)\}|\b(?:psi|Psi)_[A-Za-z0-9]+|\b(?:psi|Psi)\s*\(|\bhbar\b|\b[A-Za-z]_[A-Za-z0-9]+|\bsum\s*\(|\bln\s*\(|\bexp\s*\(|\bpartial\b|\b(?:pmatrix|bmatrix|matrix|array)\b|[∂∇ΔΩβλμψΨℏ→≤≥±≠∞]|\b[A-Z][A-Za-z0-9']*\s*=|\^[0-9²³]+|_[{(]?[A-Za-z0-9,]+[})]?)/;

function sanitizeGeneratedExerciseText(value = "") {
  return String(value || "")
    .replace(/\b(?:as|according to)\s+(?:studied|presented|described|discussed)\s+(?:by|with)\s+(?:prof\.?|professor)\s+mario\s+reis,?\s*/gi, "")
...
function isSimpleInlineMath(value = "") {
  ...
  if (hasProseCue(text) || hasPlainProseWord(text)) return false;
...
function hasPlainProseWord(value = "") {
  const text = String(value || "")
    .replace(/\\[A-Za-z]+/g, " ")
    .replace(/[A-Za-z]_[A-Za-z0-9]+/g, " ")
    .replace(/[A-Za-z]\^[A-Za-z0-9]+/g, " ");
  return /\b[A-Za-zÀ-ÿ]{3,}\b/u.test(text);
}

function shouldPreserveSquareBrackets(value = "") {
  const text = String(value || "").trim();
  return /,/.test(text) && looksMathy(text);
}

function compactDisplayMathBlocks(value = "") {
  return String(value || "").replace(/\\\[([\s\S]*?)\\\]/g, (_match, content) => `\\[${cleanupEquation(content)}\\]`);
}
...
function repairGeneratedMathDelimiters(value = "") {
  ...
  while ((match = tokens.exec(text))) {
    ...
    if (token === "\\(") {
      ...
      if (displayBalance === 0 && inlineBalance > 0) {
        inlineBalance -= 1;
        output.push(token);
      }
    }
  ...
  text = output.join("");
  if (displayBalance > 0) text += "\\]".repeat(displayBalance);
  if (inlineBalance > 0) text += "\\)".repeat(inlineBalance);
...
function splitProseInsideInlineMath(value = "") {
  ...
  return String(value || "").replace(/\\\(([^\n]*?)\\\)/g, (match, content) => {
    ...
    if (!mathPart || !prosePart || !looksMathy(mathPart) || mathWords > 2) {
      return match;
    }
...
function replaceOutsideMathSegments(value = "", transform = (segment) => segment) {
  const segments = [];
  const masked = String(value || "").replace(/\\\[[\s\S]+?\\\]|\\\([\s\S]+?\\\)/g, (segment) => {
    const token = `@@QM_MATH_${segments.length}@@`;
    segments.push(segment);
    return token;
  });
  return transform(masked).replace(/@@QM_MATH_(\d+)@@/g, (_match, index) => segments[Number(index)] || "");
}
...
function flattenMathDelimitersInsideSegments(value = "") {
  let text = String(value || "");
  let previous = "";
  while (text !== previous) {
    previous = text;
    text = text
      // A display expression is already a MathJax region. Inline delimiters
      // inside it are invalid and must become ordinary LaTeX content.
      .replace(/\\\[([\s\S]*?)\\\]/g, (_match, content) =>
        `\\[${String(content).replace(/\\\(([\s\S]*?)\\\)/g, "$1").replace(/\\\[([\s\S]*?)\\\]/g, "$1")}\\]`
      )
      // The reciprocal case is also invalid. Flatten it rather than emitting
      // a nested region that MathJax cannot parse.
      .replace(/\\\(([^\n]*?)\\\)/g, (_match, content) =>
        `\\(${String(content).replace(/\\\[([\s\S]*?)\\\]/g, "$1").replace(/\\\(([\s\S]*?)\\\)/g, "$1")}\\)`
      );
  }
...
function expandQuantumMacros(value = "") {
  return String(value || "")
    .replace(/\\braket\{([^{}]+)\}\{([^{}]+)\}/g, "\\left\\langle $1 \\middle| $2 \\right\\rangle")
    .replace(/\\bra\{([^{}]+)\}/g, "\\left\\langle $1 \\right|")
    .replace(/\\ket\{([^{}]+)\}/g, "\\left| $1 \\right\\rangle");
}

function wrapBareLatexExpressions(value = "") {
  const command = /\\(?:left|right|frac|partial|nabla|psi|Psi|phi|Phi|hbar|hat|langle|rangle|dagger|bra|ket|braket|delta|otimes|operatorname|mathrm|text|begin|end|sum|int|sqrt|cdot|ln|exp|to|rightarrow|le|ge|neq|infty)(?![A-Za-z])/;
  const run = /(^|[^A-Za-zÀ-ÿ\\])((?:(?:\\(?:text|mathrm)\s*\{[^}]*\})|(?:\\[A-Za-z]+)|(?:[A-Za-z](?![A-Za-zÀ-ÿ]))|(?:\d+(?:\.\d+)?)|[\s_{}()[\]=+\-*/<>.,|]){4,})(?=$|[^A-Za-zÀ-ÿ])/g;
  return replaceOutsideMathSegments(value, (segment) => segment.replace(run, (match, lead, candidate) => {
    if (!command.test(candidate)) return match;
    const cleaned = cleanupEquation(candidate);
    if (hasProseCue(cleaned)) return match;
    return cleaned && command.test(cleaned) ? `${lead}\\(${cleaned}\\)` : match;
  }));
}

function wrapBareBracketedMath(value = "") {
  return replaceOutsideMathSegments(value, (segment) =>
    segment.replace(/(^|[^\\])\[\s*([^[\]\n]{3,180})\s*\]/g, (match, lead, snippet) => {
...
function wrapBareMathTokens(value = "") {
  return replaceOutsideMathSegments(wrapBareLatexExpressions(wrapBareBracketedMath(value)), (segment) => segment
...
function wrapCompoundAssignments(value = "") {
  return replaceOutsideMathSegments(value, (segment) =>
    segment.replace(
      /\b([A-Za-z])\s*=\s*([A-Za-z])\s+and\s+([A-Za-z])\s*=\s*([A-Za-z])(?=[:.,;\n]|$)/g,
      (_match, leftSymbol, leftValue, rightSymbol, rightValue) =>
        `\\(${leftSymbol} = ${leftValue}\\) and \\(${rightSymbol} = ${rightValue}\\)`
    )
  );
}
...
function normalizeStandaloneMathLine(line = "") {
  ...
  if (bracketMathMatch && looksMathy(bracketMathMatch[1]) && !shouldPreserveSquareBrackets(bracketMathMatch[1])) {
...
function normalizeMathText(value = "") {
  const source = compactDisplayMathBlocks(collapseOverescapedLatex(
    wrapCompoundAssignments(splitProseInsideInlineMath(repairGeneratedMathDelimiters(sanitizeGeneratedExerciseText(value))))
      .replace(/\r\n?/g, "\n")
  ));
  const normalized = wrapBareMathTokens(
    replaceOutsideMathSegments(source
    ...
    .replace(/(^|[^\\])\[\s*([^[\]\n]{3,180})\s*\]/g, function (match, lead, snippet) {
      if (!looksMathy(snippet)) return match;
      return `${lead}\\(\\left[${latexifySnippet(snippet)}\\right]\\)`;
    })
    , (segment) => segment.replace(/([A-Za-z][A-Za-z0-9']*(?:_[A-Za-z0-9]+)?\s*=\s*[^.,;\n]+)(?=[.,;\n]|$)/g, function (_match, snippet) {
  ...
  const postProcessed = splitEmbeddedDisplayMath(expandQuantumMacros(flattenMathDelimitersInsideSegments(splitProseInsideInlineMath(cleanupNestedMathDelimiters(normalized)))));
  return collapseOverescapedLatex(postProcessed)
...
function normalizeExerciseMathLocally(exercise = {}) {
  return {
    title: sanitizeGeneratedExerciseText(exercise.title || "Exercise").replace(/\s+/g, " ").trim() || "Exercise",
    statement: normalizeMathText(exercise.statement || ""),
    solution: normalizeMathText(exercise.solution || "")
  };
}

// Exported for deterministic smoke tests; request handling remains the only
// production entry point for generation.
export function normalizeExerciseMathForTest(exercise = {}) {
  return normalizeExerciseMathLocally(exercise);
}
...
function buildPrompt({
  ...
  const sourcePrompt = buildAiExerciseContextPrompt(contextPackage);
...
- When a canonical book reference is supplied, treat it as the authority for the physics and notation.
- Create a genuinely new exercise; do not copy a question or a solution from the supplied books.
...
- When writing matrices, use valid LaTeX environments such as \\begin{pmatrix} ... \\end{pmatrix}; separate columns with & and rows with \\\\.
- Never write a matrix as a flat parenthesized list such as (1 0 0 1).
...
- Prefer standard \\langle, \\rangle and vertical-bar notation for bras and kets; do not rely on custom LaTeX macros.
- Respond ONLY with valid JSON.
- JSON requires each LaTeX backslash to be escaped. For example, return "\\\\(E=mc^2\\\\)" in JSON, not a single unescaped backslash.
- Do not use Markdown.
...
- Escape every LaTeX backslash for JSON (for example, "\\\\[\\\\frac{a}{b}\\\\]").
...
const RAW_LATEX_JSON_ESCAPE_PATTERN =
  /(^|[^\\])\\(?=(?:[\[\]()]|(?:alpha|bar|begin|beta|braket|bra|cdot|cos|dagger|Delta|delta|ell|epsilon|exp|frac|gamma|ge|hat|hbar|infty|int|kappa|ket|lambda|langle|le|left|ln|mathit|mathbf|mathrm|mu|nabla|neq|nu|Omega|overline|partial|Phi|phi|pm|rho|rangle|rightarrow|right|sigma|sin|sqrt|sum|tau|text|theta|Theta|tilde|times|to|varepsilon|vec)\b))/g;
...
async function selectValidationReports({ env = process.env, accessToken = "", status = "pending", id = "" }) {
  ...
  if (id) {
    ...
    if (normalizedStatus !== "all") {
      params.set("review_status", `eq.${normalizeReviewStatus(normalizedStatus)}`);
    }
  }
...
export async function handleExerciseRequest({
  ...
  if (!isChapterExerciseEligible(sanitizedBody.chapterId)) {
    return {
      status: 403,
      body: { error: "Exercises are unavailable while this chapter is under editorial review." }
    };
  }
  ...
  try {
    ...
    if ((requiresRemoteMathRefinement(normalized) && needsMathFormatting(rawExercise)) || !mathContract.ok) {
    ...
    return {
      ...
      body: {
        ...
        modelAttempts: [{ model: result.model || model, status: result.status, ok: true }],
        sourceReferences: contextPackage.sourceReferences || [],
        contextPackageMeta: contextPackage.contextPackageMeta || {},
        validationMemoryCount: validationMemory.length,
        validationMemoryApplied: validationMemory.length > 0,
        bookContextApplied: Boolean(contextPackage.primarySource),
        bookReference: contextPackage.primarySource ? {
          chapterId: contextPackage.primarySource.chapterId,
          itemId: contextPackage.primarySource.itemId,
          pageStart: contextPackage.primarySource.pageStart,
          pageEnd: contextPackage.primarySource.pageEnd
        } : null,
        solutionGuidanceApplied: Boolean(contextPackage.solutionGuidance),
