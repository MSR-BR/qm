...
function repairGeneratedMathDelimiters(value = "") {
  ...
  while ((match = tokens.exec(text))) {
    ...
    const token = match[0];
    if (token === "\\(") { inlineBalance += 1; output.push(token); }
    else if (token === "\\)") { if (inlineBalance > 0) { inlineBalance -= 1; output.push(token); } }
    else if (token === "\\[") { displayBalance += 1; output.push(token); }
    else if (token === "\\]") { if (displayBalance > 0) { displayBalance -= 1; output.push(token); } }
...
function splitProseInsideInlineMath(value = "") {
  ...
  return String(value || "").replace(/\\\(([^\n]*?)\\\)/g, (match, content) => {
    ...
    if (!mathPart || !prosePart || !looksMathy(mathPart) || countWords(mathPart) > 3) {
      return match;
    }
...
function normalizeMathText(value = "") {
  const normalized = wrapBareMathTokens(
    replaceOutsideMathSegments(splitProseInsideInlineMath(repairGeneratedMathDelimiters(sanitizeGeneratedExerciseText(value)))
    .replace(/\r\n?/g, "\n")
    .replace(/\\\\/g, "\\")
  ...
  return expandQuantumMacros(flattenMathDelimitersInsideSegments(splitProseInsideInlineMath(cleanupNestedMathDelimiters(normalized))))
    .replace(/\\\\(?=[()[\]]|[A-Za-z])/g, "\\")
