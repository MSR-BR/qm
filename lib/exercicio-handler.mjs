...
const plainMathPattern =
  /(?:\b(?:psi|Psi)_[A-Za-z0-9]+|\b(?:psi|Psi)\s*\(|\bhbar\b|\b[A-Za-z]_[A-Za-z0-9]+|\bsum\s*\(|\bln\s*\(|\bexp\s*\(|\bpartial\b|[∂∇ΔΩβλμψΨℏ→≤≥±≠∞]|\b[A-Z][A-Za-z0-9']*\s*=|\^[0-9²³]+|_[{(]?[A-Za-z0-9,]+[})]?)/;
...
function wrapBareLatexExpressions(value = "") {
  const command = /\\(?:left|right|frac|partial|nabla|psi|Psi|phi|Phi|hbar|hat|langle|rangle|dagger|bra|ket|braket|delta|otimes|operatorname|mathrm|text|sum|int|sqrt|cdot|ln|exp|to|rightarrow|le|ge|neq|infty)(?![A-Za-z])/;
...
function normalizeMathText(value = "") {
  const normalized = wrapBareMathTokens(
    replaceOutsideMathSegments(wrapCompoundAssignments(splitProseInsideInlineMath(repairGeneratedMathDelimiters(sanitizeGeneratedExerciseText(value))))
    .replace(/\r\n?/g, "\n")
    .replace(/\\\\/g, "\\")
  ...
  return splitEmbeddedDisplayMath(expandQuantumMacros(flattenMathDelimitersInsideSegments(splitProseInsideInlineMath(cleanupNestedMathDelimiters(normalized)))))
    .replace(/\\\\(?=[()[\]]|[A-Za-z])/g, "\\")
