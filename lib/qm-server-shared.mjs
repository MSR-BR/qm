...
const DEFAULT_NEXT_ACTION = {
  type: "choose_chapter",
  label: "Choose your first chapter",
  href: "index.html?view=chapters",
  reason: "Your journey starts empty. Choose a chapter, complete reviewed sections, or take an assessment to receive recommendations."
};
...
export function isGamificationRpcEnabled(env = process.env) {
  return TRUE_ENV_VALUES.has(String(env.QM_GAMIFICATION_RPC_MODE || "").trim().toLowerCase());
}
