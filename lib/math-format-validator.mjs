...
const RAW_LATEX_COMMAND_PATTERN =
  /\\(?:frac|partial|nabla|Delta|Omega|mu|lambda|beta|gamma|delta|psi|Psi|Phi|phi|hbar|hat|langle|rangle|dagger|bra|ket|braket|begin|end|sum|int|sqrt|cdot|ln|exp|operatorname|mathrm|text|to|rightarrow|le|ge|neq|infty|varepsilon|epsilon|kappa|theta|Theta|alpha|sigma|rho)\b/;
const BARE_MATH_TOKEN_PATTERN =
  /\b(?:psi(?:_[A-Za-z0-9]+)?\s*\(|Psi(?:_[A-Za-z0-9]+)?\s*\(|hbar\b|ket\s*\(|bra\s*\(|braket\s*\(|pmatrix\b|bmatrix\b|matrix\b|array\b|k_[A-Za-z0-9]+|[A-Za-z]_[A-Za-z0-9]+|[A-Za-z]\^[A-Za-z0-9]+|sum\s*\(|partial\b|d\s*\/\s*d[A-Za-z]|[A-Z][A-Za-z0-9']*\s*=\s*[^.,;\n]{1,80})\b|[∂∇ΔΩβλμψΨℏ→≤≥±≠∞]/;
...
function validateMathSegment(segment = "", index = 0) {
  ...
  const proseProbe = content
    .replace(/\\(?:text|mathrm|operatorname)\s*\{[^}]*\}/g, "")
    .replace(/\\[A-Za-z]+\s*(?:\{[^}]*\})?/g, " ")
    .replace(/[{}_^=+\-*/0-9.,;:|()[\]\\]/g, " ");
  if (/\b(?:and|we|set|between|where|using|given|show|calculate|determine|evaluate|explain|prove|component|operator|state|particle|wave|energy|momentum|position|basis)\b/i.test(proseProbe)) {
    errors.push({
      code: "prose_inside_math_segment",
      message: `Segmento matematico ${index + 1} contem texto comum dentro dos delimitadores.`
    });
  }
