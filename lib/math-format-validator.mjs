...
const RAW_LATEX_COMMAND_PATTERN =
  /\\(?:frac|partial|nabla|Delta|Omega|mu|lambda|beta|gamma|delta|psi|Psi|Phi|phi|hbar|hat|langle|rangle|dagger|bra|ket|braket|sum|int|sqrt|cdot|ln|exp|operatorname|mathrm|text|to|rightarrow|le|ge|neq|infty|varepsilon|epsilon|kappa|theta|Theta|alpha|sigma|rho)\b/;
const BARE_MATH_TOKEN_PATTERN =
  /\b(?:psi(?:_[A-Za-z0-9]+)?\s*\(|Psi(?:_[A-Za-z0-9]+)?\s*\(|hbar\b|ket\s*\(|bra\s*\(|braket\s*\(|k_[A-Za-z0-9]+|[A-Za-z]_[A-Za-z0-9]+|[A-Za-z]\^[A-Za-z0-9]+|sum\s*\(|partial\b|d\s*\/\s*d[A-Za-z]|[A-Z][A-Za-z0-9']*\s*=\s*[^.,;\n]{1,80})\b|[∂∇ΔΩβλμψΨℏ→≤≥±≠∞]/;
