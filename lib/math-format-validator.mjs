...
function validateMathSegment(segment = "", index = 0) {
  ...
  const proseProbe = content
    ...
    .replace(/\\[A-Za-z]+\s*(?:\{[^}]*\})?/g, " ")
    .replace(/[{}_^=+\-*/0-9.,;:|()[\]\\]/g, " ");
  if (/\b(?:between|where|using|given|show|calculate|determine|evaluate|explain|prove|component|operator|state|particle|wave|energy|momentum|position|basis)\b/i.test(proseProbe)) {
    errors.push({
